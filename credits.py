...
async def deduct_credits(user_id: int, amount: int, reason: str) -> bool:
    ...
    Atomic by construction: the balance check and the decrement happen in a
    single UPDATE ... WHERE balance >= $2 statement, so there's no gap
    between "read balance" and "write new balance" for two concurrent calls
    to race through. (A prior version used SELECT ... FOR UPDATE followed by
    a separate UPDATE, but each went through its own db_execute() call — its
    own pool.acquire() and own implicit, auto-committed transaction — so the
    row lock was released before the UPDATE ever ran. Two near-simultaneous
    deductions could both read a sufficient balance and both apply,
    overdrawing the account. Same class of bug as the payment double-credit
    fixed in payment.fulfill_payment(); fixed here the same way, by making
    the DB do the check-and-set as one round trip instead of two.)
    """
    now = int(time.time())
    ...
    if new_balance is None:
        # Either the user has no row yet, or balance < amount — caller should
        # have called ensure_user_credits() first for the "no row" case.
        return False
