...
# Known music/video platform hosts we're willing to hand a raw user-supplied
# URL to yt-dlp for. yt-dlp's generic extractor will fetch *any* URL given
# to it — without this allowlist, a user could send an arbitrary internal
# or cloud-metadata URL (e.g. http://169.254.169.254/...) as a "track link"
# and the bot's own server would make that outbound request on their behalf.
# Odesli (called just above this list's only call site) is exempt: it never
# fetches the URL itself, only forwards it as a query param to api.song.link.
...
class TrackManager:
    ...
    async def _process_url_input(self, url: str) -> tuple[str, str, str, dict]:
        ...
        if not _is_allowed_music_url(url):
            logger.info("Refusing to pass non-allowlisted URL to yt-dlp: %s", url[:120])
            return "", "", "", {}
