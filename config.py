...
# WEBHOOK_SECRET_TOKEN: passed to Application.run_webhook(secret_token=...).
# PTB then only accepts requests whose X-Telegram-Bot-Api-Secret-Token header
# matches — without it, WEBHOOK_PATH is a fixed, guessable URL
# (default "/webhook") and *anything* on the internet can POST a
# hand-crafted Update JSON (any from.id, including a spoofed admin id) and
# every admin-only handler (is_admin() in this module trusts
# update.effective_user.id unconditionally) will act on it.
# Generate one with: python -c "import secrets; print(secrets.token_urlsafe(32))"
WEBHOOK_SECRET_TOKEN = os.environ.get("WEBHOOK_SECRET_TOKEN", "")

if WEBHOOK_URL and not WEBHOOK_SECRET_TOKEN:
    logger.warning(
        "WEBHOOK_URL is set but WEBHOOK_SECRET_TOKEN is not — the webhook endpoint "
        "is unauthenticated. Anyone who finds %s can POST forged Telegram updates "
        "(including a spoofed admin id) directly to the bot. Set WEBHOOK_SECRET_TOKEN.",
        WEBHOOK_PATH,
    )
